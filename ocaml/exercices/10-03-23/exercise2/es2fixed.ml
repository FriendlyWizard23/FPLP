...
let convert_all temp scales= 
	let rec convert newlist= function
		| [] -> newlist 
		| h::tl -> convert ((temperature_manager temp h)::(newlist))tl
	in convert [] scales;;
...
let generate_table default_value scales = 
	...
	| [] -> list
	| h::tl -> generate (List.append(list)(convert_all({value=default_value;scale=h})scales))tl
	in generate [] scales 
;;
